# Filename Ch2_SmileyPopHitCounter.py
...
blap = pygame.mixer.Sound("blip.wav")
# настраиваем надпись результатов
font = pygame.font.SysFont("Arial", 24)  # шрифт отрисовки
WHITE = (255, 255, 255)  # цвет отрисовки
count_smileys = 0
count_popped = 0

hits = 0 # количество попаданий мышкой по шарику
miss = 0 # количество НЕ попаданий мышкой по шарику
...
while keep_going:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if pygame.mouse.get_pressed()[0]:  # Левая кнопка мыши - рисовать
                mousedown = True
            elif pygame.mouse.get_pressed()[2]:  # Правая кнопка мыши - лопать
    ...
    clock.tick(60)
    draw_string = "Шариков создано: " + str(count_smileys)  # отображение надписи
    draw_string += " - Шариков лопнуто: " + str(count_popped)  # отображение надписи
    draw_string2 = "Щелчков мыши: " + str(round(hits+miss))
    draw_string2 += "Попаданий: " + str(hits) + "Промахов: " + str(miss)
    if (count_smileys > 0):
        draw_string += "  Процент: "
    ...
    # Hit/Miss counter
    hit_string = "Total Clicks: " + str(hits+miss)
    hit_string += " - Hits: " + str(hits)
    if (hits+miss > 0):
        hit_string += " - Percent: "
        hit_string += str(round(hits*100/(hits+miss), 1)) + "%"
