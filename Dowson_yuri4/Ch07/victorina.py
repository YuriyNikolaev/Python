...
def next_block(the_file):
	...
	category = next_line(the_file) # Тема
	question = next_line(the_file) # Вопрос
	answers = []                   # Варианты ответов
	for i in range(4):
		answers.append(next_line(the_file))
	correct = next_line(the_file)  # правильны ответ
	if correct:
		correct = correct[0]
	explanation = next_line(the_file) # комментарий
...
def main():
	...
	trivia_file = open_file("triviа_ru0.txt", "r")
	title = next_line(trivia_file) # читатет заглавие игры из файла
	welcome(title)                 # выводит заглавие игры на экран
	score = 0
