...
class BJ_Hand(cards.Hand):
	...
	def total(self):
		...
		# определяем, есть ли туз на руказ у игрока
		contains_ace = False
		for card in self.cards:
			if card.value == BJ_Card.ACE_VALUE:
				contains_ace = True
		
		# если на руках есть туз и сумма очков превышает 11, будем считать туз за 11 очков
		if contains_ace and t <= 11:
			# прибавить нужо лишь 10, потому что единица уже вошла в общую сумму
			t += 10
		return t
...
class BJ_Game(object):
	...
	def play(self):
		...
		self.dealer.flip_first_card() # первая из карт, сданных дилеру, поворачивается
		# рубашкой вверх
		for player in self.players:
			print(player)
		print(self.dealer)
